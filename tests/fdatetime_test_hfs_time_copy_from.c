/*
 * Date and time library HFS time copy from testing program
 *
...
/* Tests copying a HFS time from a byte stream
     ...
     int expected_result )
{
	...
	else if( byte_order == LIBFDATETIME_ENDIAN_LITTLE )
	{
        ...
        fprintf(
         stdout,
         "Testing copying HFS time from byte stream: 0x%08" PRIjx " of size: %" PRIzd " and byte order: %s\t",
