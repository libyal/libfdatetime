#!/bin/bash
# Library POSIX time copy from testing script
#
# Version: 20160217
...
TEST_COPY_FROM="./${TEST_PREFIX}_test_posix_time_copy_from";

if ! test -x ${TEST_COPY_FROM};
then
	TEST_COPY_FROM="${TEST_PREFIX}_test_posix_time_copy_from.exe";
fi
