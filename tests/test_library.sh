...
LIBRARY_TESTS="date_time_values error fat_date_time filetime floatingtime hfs_time nsf_timedate posix_time support systemtime";
...
run_test()
{
	...
	local TEST_EXECUTABLE="./fdatetime_test_${TEST_NAME}";
...
run_test_with_input()
{
	...
	run_test_on_input_directory "libfdatetime" "${TEST_DESCRIPTION}" "default" "${OPTION_SETS}" "${TEST_EXECUTABLE}" "input" "${INPUT_GLOB}";
	local RESULT=$?;
