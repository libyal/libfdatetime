#!/bin/bash
# Library systemtime copy from testing script
#
# Version: 20160217
...
TEST_COPY_FROM="./${TEST_PREFIX}_test_systemtime_copy_from";

if ! test -x ${TEST_COPY_FROM};
then
	TEST_COPY_FROM="${TEST_PREFIX}_test_systemtime_copy_from.exe";
fi
