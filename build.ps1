# Script that builds libfdatetime
#
# Version: 20260608
...
Else
{
	...
	If (-Not (Test-Path "${VSSolutionPath}"))
	{
		...
		Invoke-Expression -Command "& '${Python}' ${MSVSCppConvert} --output-format ${VisualStudioVersion} ${VSToolsOptions} msvscpp\libfdatetime.sln 2>&1" | %{ "$_" }
	}
}
If (${VisualStudioVersion} -eq "2026")
{
	$VSSolutionFile = "${VSSolutionPath}\libfdatetime.slnx"
}
Else
{
	$VSSolutionFile = "${VSSolutionPath}\libfdatetime.sln"
}
