...
AC_DEFUN([LIBFDATETIME_CHECK_FUNC_LANGINFO_CODESET],
 ...
 AS_IF(
  ...
  [AC_CACHE_CHECK(
   ...
   [ac_cv_libfdatetime_langinfo_codeset],
   [AC_LANG_PUSH(C)
   AC_LINK_IFELSE(
    [AC_LANG_PROGRAM(
     [[#include <langinfo.h>]],
     [[char* charset = nl_langinfo( CODESET );]] )],
    [ac_cv_libfdatetime_langinfo_codeset=yes],
    [ac_cv_libfdatetime_langinfo_codeset=no])
   AC_LANG_POP(C) ]) ],
  [ac_cv_libfdatetime_langinfo_codeset=no])

 AS_IF(
  [test "x$ac_cv_libfdatetime_langinfo_codeset" = xyes],
  [AC_DEFINE(
...
AC_DEFUN([LIBFDATETIME_CHECK_FUNC_PRINTF_JD],
 [AC_MSG_CHECKING(
 ...
 AC_LINK_IFELSE(
  [AC_LANG_PROGRAM(
   [[#include <stdio.h>]],
   [[printf( "%jd" ); ]] )],
  [ac_cv_libfdatetime_have_printf_jd=no],
  [ac_cv_libfdatetime_have_printf_jd=yes])

 dnl Second try to see if compilation and linkage with a parameter succeeds
 AS_IF(
  [test "x$ac_cv_libfdatetime_have_printf_jd" = xyes],
  [AC_LINK_IFELSE(
   [AC_LANG_PROGRAM(
    [[#include <sys/types.h>
#include <stdio.h>]],
...
if( ( string[ 0 ] != '1' ) || ( string[ 1 ] != '0' ) ) return( 1 ); ]] )],
    [ac_cv_libfdatetime_have_printf_jd=yes],
    [ac_cv_libfdatetime_have_printf_jd=no],
    [ac_cv_libfdatetime_have_printf_jd=undetermined])
   ])
 ...
 AS_IF(
  ...
  [AC_MSG_RESULT(
   [$ac_cv_libfdatetime_have_printf_jd]) ])
 ])

dnl Function to detect whether printf conversion specifier "%zd" is available
AC_DEFUN([LIBFDATETIME_CHECK_FUNC_PRINTF_ZD],
 [AC_MSG_CHECKING(
 ...
 AC_LINK_IFELSE(
  [AC_LANG_PROGRAM(
   [[#include <stdio.h>]],
   [[printf( "%zd" ); ]] )],
  [ac_cv_libfdatetime_have_printf_zd=no],
  [ac_cv_libfdatetime_have_printf_zd=yes])

 dnl Second try to see if compilation and linkage with a parameter succeeds
 AS_IF(
  [test "x$ac_cv_libfdatetime_have_printf_zd" = xyes],
  [AC_LINK_IFELSE(
   [AC_LANG_PROGRAM(
    [[#include <sys/types.h>
#include <stdio.h>]],
...
if( ( string[ 0 ] != '1' ) || ( string[ 1 ] != '0' ) ) return( 1 ); ]] )],
    [ac_cv_libfdatetime_have_printf_zd=yes],
    [ac_cv_libfdatetime_have_printf_zd=no],
    [ac_cv_libfdatetime_have_printf_zd=undetermined])
   ])
 ...
 AS_IF(
  ...
  [AC_MSG_RESULT(
   [$ac_cv_libfdatetime_have_printf_zd]) ])
 ])
